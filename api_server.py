...
# Bazani yaratish (jadvallarni hosil qilish)
...
async def onboard_user(req: OnboardRequest, db: Session = Depends(get_db)):
    ...
    if user:
        ...
        new_user = models.User(
            ...
            level=req.level,
            points=10 # Bonus for registering
        )
        db.add(new_user)
        db.commit()
        db.refresh(new_user)
        return {"status": "success", "message": "User registered", "points": new_user.points}
...
@app.post("/api/users/add_points")
async def add_points(req: PointsRequest, db: Session = Depends(get_db)):
    user = db.query(models.User).filter(models.User.telegram_id == req.telegram_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Foydalanuvchi topilmadi")
    
    user.points += req.points_to_add
    db.commit()
    db.refresh(user)
    return {"status": "success", "new_points": user.points, "reason": req.reason}
